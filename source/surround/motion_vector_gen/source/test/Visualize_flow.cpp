    ...
    const float bottomAngle)
{
  ...
  VLOG(1) << "FLAGS_side_alpha_feather_size is " << FLAGS_side_alpha_feather_size << "\n";
  VLOG(1) << "leftAngle" << leftAngle << "right=" << rightAngle << "topAngle=" << topAngle << "bottomAngle=" << bottomAngle << "\n";
  ...
  bicubicRemapToSpherical(
    ...
    Mat temp_dst ;
    cvtColor(dst, temp_dst, CV_BGRA2BGR);
    VLOG(1) << "";
}
    ...
    vector<Mat> &projectionImages)
{
  ...
  //cv::Size_<int> s1 = camImages.size();
  VLOG(1) << " THE IMAGE SIZE IS " << camImages.size();
  ...
  VLOG(1) << " hfov " << hRadians << " vfov " << vRadians << "\n";
...
//   Mat temp_dst1;
//   cvtColor(dst1, temp_dst1, CV_BGRA2BGR);
//   VLOG(1) << "";
//   Mat temp_dst2;
//   cvtColor(dst2, temp_dst2, CV_BGRA2BGR);
  VLOG(1) << "";
...
// sgunnam
// Mat tempL = lazyNovelChunksLR.first.clone();
// cvtColor(tempL, tempL, CV_BGRA2BGR);
// Mat tempR = lazyNovelChunksLR.second.clone();
// cvtColor(tempR, tempR, CV_BGRA2BGR);
// VLOG(1) << "";
    ...
    double &novelViewRuntime)
{
  ...
  for (int leftIdx = 0; leftIdx < projectionImages.size(); ++leftIdx) //projectionImages.size()
  {
  ...
  // sgunnam debug
  vector<Mat> panoChunksLdebug(projectionImages.size(), Mat());
  for (int leftIdx = 0; leftIdx < projectionImages.size(); ++leftIdx) // sgunnam projectionImages.size()
  {
      Mat tempL, tempR ;
      tempL = panoChunksL[leftIdx].clone();
      tempR = panoChunksR[leftIdx].clone();
      cvtColor(tempL, tempL, CV_BGRA2BGR);
      cvtColor(tempR, tempR, CV_BGRA2BGR);
      //imshow("chunk",panoChunksL[leftIdx]);
...
//   VLOG(1) << FLAGS_frame_number << "\n";
//   VLOG(1) << FLAGS_output_data_dir << "\n";
//   VLOG(1) << FLAGS_prev_frame_data_dir << "\n";
//   VLOG(1) << FLAGS_output_cubemap_path << "\n";
//   VLOG(1) << FLAGS_output_equirect_path << "\n";
//   VLOG(1) << FLAGS_interpupilary_dist << "\n";
//   VLOG(1) << FLAGS_side_alpha_feather_size << "\n";
//   VLOG(1) << FLAGS_std_alpha_feather_size << "\n";
//   VLOG(1) << FLAGS_save_debug_images << "\n";
//   VLOG(1) << FLAGS_sharpening << "\n";
//   VLOG(1) << FLAGS_enable_top << "\n";
//   VLOG(1) << FLAGS_enable_bottom << "\n";
//   VLOG(1) << FLAGS_enable_pole_removal << "\n";
//   VLOG(1) << FLAGS_bottom_pole_masks_dir << "\n";
//   VLOG(1) << FLAGS_side_flow_alg << "\n";
//   VLOG(1) << FLAGS_polar_flow_alg << "\n";
//   VLOG(1) << FLAGS_poleremoval_flow_alg << "\n";
//   VLOG(1) << FLAGS_zero_parallax_dist << "\n";
//   VLOG(1) << FLAGS_eqr_width << "\n";
//   VLOG(1) << FLAGS_eqr_height << "\n";
//   VLOG(1) << FLAGS_final_eqr_width << "\n";
//   VLOG(1) << FLAGS_final_eqr_height << "\n";
//   VLOG(1) << FLAGS_cubemap_width << "\n";
//   VLOG(1) << FLAGS_cubemap_height << "\n";
//   VLOG(1) << FLAGS_cubemap_format << "\n";

//FLAGS_rig_json_file = "/media/gunman/Data/thesis/stitch/available_solutions/Surround360/jetsonData/rgb_2cams/config/camera_rig.json";
FLAGS_rig_json_file = "/media/gunman/Data/thesis/stitch/available_solutions/Surround360/jetsonData/rgb_2cams/config/camera_rig.json";
FLAGS_imgs_dir = "/media/gunman/Data/thesis/stitch/available_solutions/Surround360/jetsonData/rgb_2cams/rgb";
FLAGS_frame_number = "000001";
FLAGS_output_data_dir = "/media/gunman/Data/thesis/stitch/available_solutions/Surround360/jetsonData/rgb_2cams";
//FLAGS_prev_frame_data_dir = "000001";
FLAGS_prev_frame_data_dir = "NONE";
FLAGS_output_cubemap_path = ""; // /media/gunman/Data/thesis/stitch/available_solutions/Surround360/jetsonData/rgb_2cams/cube_frames/cube_000001.png";
FLAGS_output_equirect_path = "/media/gunman/Data/thesis/stitch/available_solutions/Surround360/jetsonData/rgb_2cams/eqr_frames/eqr_000001.png";
FLAGS_interpupilary_dist = 6.4;
FLAGS_side_alpha_feather_size = 100;
FLAGS_std_alpha_feather_size = 31;
FLAGS_save_debug_images = 1;
FLAGS_sharpening = 0.25f;
FLAGS_enable_top = false;
FLAGS_enable_bottom = false;
FLAGS_enable_pole_removal = false;
FLAGS_side_flow_alg = "pixflow_low";
FLAGS_polar_flow_alg = "pixflow_low";
FLAGS_poleremoval_flow_alg = "pixflow_low";
// FLAGS_zero_parallax_dist = 10000;
FLAGS_zero_parallax_dist = 0;
FLAGS_eqr_width = 6300;
FLAGS_eqr_height = 3072;
FLAGS_final_eqr_width = 6144;
FLAGS_final_eqr_height = 6144;
...
if (FLAGS_eqr_width % rig.getSideCameraCount() != 0)
{
  ...
  cvtColor(sphericalImageL, temp1, CV_BGRA2BGR);
  cvtColor(sphericalImageR, temp2, CV_BGRA2BGR);
  ...
  if (FLAGS_cubemap_width > 0 && FLAGS_cubemap_height > 0 && !FLAGS_output_cubemap_path.empty())
  {
