    ...
    NovelViewGenerator *novelViewGen)
{
    ...
    //     Mat flowVisLtoRColorWithLines = visualizeFlowAsVectorField(novelViewGen->getFlowLtoR(), overlapImageL);
    //     Mat flowVisRtoLColorWithLines = visualizeFlowAsVectorField(novelViewGen->getFlowRtoL(), overlapImageR);
    //     cvtColor(flowVisRtoL, flowVisRtoL, CV_GRAY2BGR);
    //     cvtColor(flowVisLtoR, flowVisLtoR, CV_GRAY2BGR);
    //     cvtColor(flowVisLtoRColorWithLines, flowVisLtoRColorWithLines, CV_BGRA2BGR);
    //     cvtColor(flowVisRtoLColorWithLines, flowVisRtoLColorWithLines, CV_BGRA2BGR);
    ...
    //     cvtColor(overlapImageL, overlapImageL, CV_BGRA2BGR);
    //     cvtColor(overlapImageR, overlapImageR,  CV_BGRA2BGR);
    //     VLOG(1) << "\n" ;
    // // sgunnam debug end
    ...
    Mat *chunkR)
{
    ...
    *chunkR = lazyNovelChunksLR.second;
    ...
    // sgunnam debug
    Mat fir = lazyNovelChunksLR.first;
    Mat sec = lazyNovelChunksLR.second;
    cvtColor(fir, fir, CV_BGRA2BGR);
    cvtColor(sec, sec, CV_BGRA2BGR);
    VLOG(1) << "\n";
}
    ...
    double &novelViewRuntime)
{
    ...
    vector<std::thread> threads;
    for (int leftIdx = 0; leftIdx < projectionImages.size(); ++leftIdx) // sgunnam projectionImages.size(); ++leftIdx)
    {
    ...
    for (int leftIdx = 0; leftIdx < projectionImages.size(); ++leftIdx) // sgunnam projectionImages.size()
    {
        cvtColor(panoChunksLdebug[leftIdx], panoChunksL[leftIdx], CV_BGRA2BGR);
    }
    // end debug
