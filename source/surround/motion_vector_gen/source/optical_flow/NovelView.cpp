  ...
  for (int y = 0; y < imageL.rows; ++y) {
    for (int x = 0; x < imageL.cols; ++x) {
      ...
      if (colorL[3] == 0 && colorR[3] == 0) {
        ...
        // const float magR = flowMagL.at<float>(y, x) / float(imageL.cols);
...
// // sgunnam debug optical flow
//   // images is first 
//   Mat lefl = leftEyeFromLeft.first;
//   Mat lefr = leftEyeFromRight.first;
//   Mat refl = rightEyeFromLeft.first;
//   Mat refr = rightEyeFromRight.first;
//   cvtColor(lefl, lefl, CV_BGRA2BGR);
//   cvtColor(lefr, lefr, CV_BGRA2BGR);
//   cvtColor(refl, refl, CV_BGRA2BGR);
//   cvtColor(refr, refr, CV_BGRA2BGR);
//   // flow is second
//   Mat leflf = leftEyeFromLeft.second;
//   Mat lefrf = leftEyeFromRight.second;
//   Mat reflf = rightEyeFromLeft.second;
//   Mat refrf = rightEyeFromRight.second;
//   Mat fir = leftEyeCombined.clone();
//   Mat sec = rightEyeCombined.clone();
...
void NovelViewGeneratorAsymmetricFlow::prepare(
  ...
  flowAlg->computeOpticalFlow(
    ...
    //sgunnam
  flowAlg->saveMotion(imageL,
                      imageR,
                      prevColorImageR,
                      prevColorImageL, motionL);
  flowAlg->saveMotion(imageR,
                      imageL,
                      prevColorImageR,
                      prevColorImageL, motionR);
  delete flowAlg;
}
