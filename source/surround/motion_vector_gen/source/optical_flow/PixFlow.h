...
struct PixFlow : public OpticalFlowInterface {
  ...
  void saveMotion(      
    ...
    motion = Mat(downscaleSize, CV_32F);
      ...
      for (int x = 0; x < rgba0byteDownscaled.cols; ++x)
      {
        motion.at<float>(y, x) =
            (fabs(rgba1byteDownscaled.at<Vec4b>(y, x)[0] - prevI1BGRADownscaled.at<Vec4b>(y, x)[0]) +
             ...
             fabs(rgba1byteDownscaled.at<Vec4b>(y, x)[2] - prevI1BGRADownscaled.at<Vec4b>(y, x)[2])) /
            (255.0f * 3.0f);
      }
      }

    //saveMotion = motion.clone();
  ...
  void computeOpticalFlow(
   ...
   // assert(prevFlow.dims == 0 || prevFlow.size() == rgba0byte.size());
    ...
    resize(rgba0byte, rgba0byteDownscaled, downscaleSize, 0, 0, CV_INTER_CUBIC);
    resize(rgba1byte, rgba1byteDownscaled, downscaleSize, 0, 0, CV_INTER_CUBIC);
    // imshow("img0", rgba0byte);
    // waitKey(0);
    // imshow("img1", rgba1byte);
    // waitKey(0);
    // VLOG(1) << " THE IMAGE SIZE FOR OPTICAL FLOW IS " << rgba0byte.size() << "\n" ; 
    Mat motion = Mat(downscaleSize, CV_32F);
    if (prevFlow.dims > 0) {
      // imshow("result", prevFlow);
      // waitKey(0);
      usePrevFlowTemporalRegularization = true;
  ...
  //   // sgunnam debug - motion detection
  //   if (prevFlow.dims > 0)
  //   {
  //     Mat motionWr = Mat(downscaleSize, CV_32F);
  //     // do motion detection vs. previous frame's images
  //     for (int y = 0; y < rgba0byteDownscaled.rows; ++y)
  //     {
  //       for (int x = 0; x < rgba0byteDownscaled.cols; ++x)
  //       {
  //         motionWr.at<float>(y, x) =
  //             (fabs(rgba1byteDownscaled.at<Vec4b>(y, x)[0] - prevI1BGRADownscaled.at<Vec4b>(y, x)[0]) +
  //              fabs(rgba1byteDownscaled.at<Vec4b>(y, x)[1] - prevI1BGRADownscaled.at<Vec4b>(y, x)[1]) +
  //              fabs(rgba1byteDownscaled.at<Vec4b>(y, x)[2] - prevI1BGRADownscaled.at<Vec4b>(y, x)[2])) /
  //             (3.0f);
  ...
  //     VLOG(1) << "\n";
  // }
  // Mat rgba1byteDownscaledtemp = rgba1byteDownscaled.clone();
  // Mat prevI1BGRADownscaledtemp = prevI1BGRADownscaled.clone();
  // cvtColor(rgba1byteDownscaled, rgba1byteDownscaled, CV_BGRA2BGR);
  // cvtColor(prevI1BGRADownscaledtemp, prevI1BGRADownscaledtemp, CV_BGRA2BGR);
  // VLOG(1) << "\n" ;
  ...
  // // sgunnam debug pyramid images
  //     for (int level = pyramidI0.size() - 1; level >= 0; --level)
  //     {
  //       Mat i0scl, i1scl;
  //       i0scl = pyramidI0[level];
  //       i1scl = pyramidI1[level];
  //       VLOG << "\n" ;
  //     }
  // sgunnam: comments - below scaling of previous flow has no effect ?
  if (usePrevFlowTemporalRegularization)
  {
    ...
    for (int level = 0; level < prevFlowPyramid.size(); ++level)
    {
      ...
      for (int level = pyramidI0.size() - 1; level >= 0; --level)
      {
        ...
        if (level > 0)
        { // scale the flow up to the next size
...
// sgunnam flow visualization
// if (true)//(hint == DirectionHint::RIGHT)
// {
//   Mat tempflow = flow.clone() ;
//   // I0 is left, I1 right
//   Mat flowVisLtoR = visualizeFlowAsGreyDisparity(tempflow);
//   Mat flowVisLtoRColorWheel = visualizeFlowColorWheel(tempflow);
//   Mat flowVisLtoRColorWithLinesI0 = visualizeFlowAsVectorField(tempflow, I1);
//   Mat flowVisLtoRColorWithLinesI1 = visualizeFlowAsVectorField(tempflow, I0);
//   // cvtColor(flowVisLtoR, flowVisLtoR, CV_GRAY2BGR);
//   // cvtColor(flowVisLtoRColorWheel, flowVisLtoRColorWheel, CV_BGR2BGR);
//   VLOG(1) << "\n" ;
// }
// else if( hint == DirectionHint::LEFT)
// {
//   Mat tempflow = flow.clone();
//   // I0 is left, I1 right
//   Mat flowVisLtoR = visualizeFlowAsGreyDisparity(tempflow);
//   Mat flowVisLtoRColorWheel = visualizeFlowColorWheel(tempflow);
//   Mat flowVisLtoRColorWithLines = visualizeFlowAsVectorField(tempflow, I0);
//   cvtColor(flowVisLtoR, flowVisLtoR, CV_GRAY2BGRA);
//   cvtColor(flowVisLtoRColorWheel, flowVisLtoRColorWheel, CV_BGR2BGRA);
//   VLOG(1) << "\n";
// }
    // end sgunnam flow visualization
  ...
  inline float errorFunction(
    ...
    //           float currErr = errorFunction(I0, I1, alpha0, alpha1,
    // I0x, I0y, I1x, I1y, x, y, flow, blurredFlow, flow.at<Point2f>(y, x));
